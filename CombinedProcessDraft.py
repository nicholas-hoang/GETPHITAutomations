...
class Solution:
    ...
    def process_student_folders(self):
        self.create_student_folders()
        self.copy_evaluation_form()
        self.rename_files()
    ...
    def run(self):
        ...
        reg_file_button = tk.Button(
            root, text="Select Registration File", command=self.select_registration_file)
        reg_file_button.pack(pady=10)

        # Evaluation file button
        eval_file_button = tk.Button(
            root, text="Select Evaluation File", command=self.select_evaluation_file)
        eval_file_button.pack(pady=10)

        # Process folders button
        process_folders_button = tk.Button(
            root, text="Process Folders", command=self.process_student_folders)
        process_folders_button.pack(pady=10)
    ...
    def extract_file_url(self):
        for student_file in self.student_files:
            with open(student_file, 'rb') as fp:
                ...
                soup = BeautifulSoup(raw_message.get_body(
                    preferencelist=('html')).get_content(), 'html.parser')
                registration_link = None
                try:
                    registration_link = soup.find_all('a')[1].get('href')
                    self.student_dict[student_file] = [registration_link]

                    time_submitted = raw_message['Received'].split(';')[
                        1].strip()
                    time_submitted = datetime.datetime.strptime(
                        time_submitted, '%a, %d %b %Y %H:%M:%S %z')
                    time_submitted = time_submitted.strftime(
                        '%m-%d-%Y %H:%M:%S')
...
class StudentEvaluations:
    def __init__(self):
        self.files = []
        self.student_evals = pd.DataFrame(
            columns=['Student Name', 'Grader 1', 'Grader 2', 'Score 1', 'Score 2'])

    def prompt_user_for_folder(self):
        folder_path = input(
            "Please enter the path to the folder containing the student folders: ")
        ...
        for file in self.files:
            ...
            try:
                ...
                if data.shape[0] >= 5 and data.shape[1] >= 8:
                    ...
                    new_row = pd.DataFrame({'Student Name': [student_name], 'Grader 1': [grader_1], 'Grader 2': [
                                           grader_2], 'Score 1': [score_1], 'Score 2': [score_2], 'Average': [average]})
                    self.student_evals = pd.concat(
                        [self.student_evals, new_row], ignore_index=True)
